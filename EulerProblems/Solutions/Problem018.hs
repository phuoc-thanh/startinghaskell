-- https://projecteuler.net/problem=18

import Data.List
import Data.Maybe

findAdjVal :: (Maybe Int,Int) -> [Int] -> Int
findAdjVal _ (x:[]) = x
findAdjVal (i,v) xs
    | x > y = v + x
    | otherwise = v + y
    where x = (!!) xs (fromJust i)
          y = (!!) xs (fromJust i + 1)

pathVal :: [Int] -> [Int] -> [Int]
pathVal [] _ = []
pathVal (x:xs) (y:ys) = (findAdjVal ((elemIndex x (x:xs)), x) (y:ys)) : pathVal xs ys

-- call pathSum $ reverse triangleNum
pathSum :: [[Int]] -> [[Int]]
pathSum (x:[]) = [x]
pathSum (x:y:xs) = pathSum $ (pathVal y x) : xs

triangleNum :: [[Int]]
triangleNum = [[75],
               [95, 64],
               [17, 47, 82],
               [18, 35, 87, 10],
               [20, 04, 82, 47, 65],
               [19, 01, 23, 75, 03, 34],
               [88, 02, 77, 73, 07, 63, 67],
               [99, 65, 04, 28, 06, 16, 70, 92],
               [41, 41, 26, 56, 83, 40, 80, 70, 33],
               [41, 48, 72, 33, 47, 32, 37, 16, 94, 29],
               [53, 71, 44, 65, 25, 43, 91, 52, 97, 51, 14],
               [70, 11, 33, 28, 77, 73, 17, 78, 39, 68, 17, 57],
               [91, 71, 52, 38, 17, 14, 91, 43, 58, 50, 27, 29, 48],
               [63, 66, 04, 68, 89, 53, 67, 30, 73, 16, 69, 87, 40, 31],
               [04, 62, 98, 27, 23, 09, 70, 98, 73, 93, 38, 53, 60, 04, 23]]

-- zipWith Max, the find the max val pair-to-pair
-- ex: [2,4,8,6] -> [4,8,8]
reduce x = zipWith max x (tail x)

-- find the maximum path of 2 rows
-- with xs length = ys length + 1 (from bottom up of triangle)
sum2 xs ys = zipWith (+) ys (reduce xs)

euler18 = foldl1 sum2 (reverse triangleNum)

--https://stackoverflow.com/questions/8002252/euler-project-18-approach